...
- `src/cost_per_task/cli.py`: `cpt serve`, `run`, `label`, `report`, `compare`, `import`, `prices refresh`, `mcp`. Attempt ids are `aYYYYMMDDTHHMMSSZ-xxxx` (random suffix); `cpt label` without `--attempt` takes the task's last attempt in log order.
