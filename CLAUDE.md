...
- `src/cost_per_task/cli.py`: `cpt serve`, `run`, `label`, `report`, `compare`, `explain`, `import`, `prices refresh`, `sessions list|import|summary`, `mcp`. Attempt ids are `aYYYYMMDDTHHMMSSZ-xxxxxxxx` (random suffix); `cpt label` and `cpt explain` without `--attempt` take the task's last attempt in log order. Every command reads and writes `cpt-log.jsonl` and `cpt-labels.jsonl` by default, `cpt sessions import` included (the 0.5.0 names `sessions-log.jsonl` and `sessions-labels.jsonl` are still used when only they exist).
...
0.5.0 (2026-09-13) added the Claude Code and Cowork sessions importer, effort capture, price tables inside the package and `--prices` defaults. Changes since 0.5.0 are listed under `[Unreleased]` in CHANGELOG.md. The Anthropic path is validated live (Claude Code on Haiku, Level 1 of the testing guide on Fable 5 and 5.1) and the sessions importer on 527 real Claude Code and Cowork sessions. The repository is public on GitHub and 0.4.0 and 0.5.0 are on PyPI.
